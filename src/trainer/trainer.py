...
class Trainer:
    ...
    def save(
        ...
        if self.args.global_rank == 0:
            ...
            netG_state_dict = self.netG.module.state_dict() if hasattr(self.netG, 'module') else self.netG.state_dict()
            netD_state_dict = self.netD.module.state_dict() if hasattr(self.netD, 'module') else self.netD.state_dict()
            torch.save(netG_state_dict, os.path.join(self.args.save_dir, f"G{str(self.iteration).zfill(7)}.pt"))
            torch.save(netD_state_dict, os.path.join(self.args.save_dir, f"D{str(self.iteration).zfill(7)}.pt"))
            """
            torch.save(
